...
$bTasks20 = COption::GetOptionString("intranet", "use_tasks_2_0", "N") == "Y";
...
$arGadgetParams["TASKS_FIELDS_SHOW"] = ($arGadgetParams["TASKS_FIELDS_SHOW"]?$arGadgetParams["TASKS_FIELDS_SHOW"]:false);
...
$arGadgetParams["PATH_TO_TASK_NEW"] = (isset($arGadgetParams["PATH_TO_TASK_NEW"])?$arGadgetParams["PATH_TO_TASK_NEW"]:"/company/personal/user/#user_id#/tasks/task/".($bTasks20 ? "edit" : "create")."/0/");
...
if($arGadgetParams["SHOW"] == "Y"):
	...
	$arP = Array(
		...
		"TASKS_FIELDS_SHOW" => $arGadgetParams["TASKS_FIELDS_SHOW"],
		"FORUM_ID" => $arGadgetParams["FORUM_ID"],
	);

	if (!$bTasks20)
	{
		$arP["IBLOCK_ID"] = $arGadgetParams["IBLOCK_ID"];
		$arP["OWNER_ID"] = $arGadgetParams["OWNER_ID"];
		$arP["TASK_TYPE"] = $arGadgetParams["TASK_TYPE"];

		if ($t == "U")
			$arP["FLT_CREATED_BY"] = $GLOBALS["USER"]->GetID();

		$arP["ORDER_BY_0"] = (($o == "C") ? "TIMESTAMP_X" : (($o == "P") ? "TASKPRIORITY" : "DATE_ACTIVE_TO"));
		$arP["ORDER_DIR_0"] = (($o == "C") ? "DESC" : (($o == "P") ? "ASC" : "ASC"));

		$APPLICATION->IncludeComponent(
			"bitrix:intranet.tasks_list",

...
BX["UI"]["FileInput"] = function(id, uploadParams, elementParams, values, template)
{
	...
	this.agent = null;
	this.container = null;
	this.counters = {created : 0, images : [], files : [], lastShown : 0};
...
*/		if (values.length > 0)
	...
	handlerFilePath : function(data)
	{
		if (BX.type.isArray(data))
		{
			...
			for (var ii = 0; ii < data.length; ii++)
			{
				...
				{
					result.push({tmp_url : data[ii], real_url : data[ii]});
				}
			}
	...
	frameFiles : function(activeId)
	{
		...
		{
			this.__frameFiles = BX.delegate(function(id){
				...
				frameMaster.start(this.agent, id, this.uploadParams);
		...
		if (this.framed === true)
		{
			this.__frameFiles(activeId);
		}
		else
		{
			this.frameFilesWait();
			BX.addCustomEvent(this, "onFilesCanBeFramed", BX.proxy(function(){this.__frameFiles(activeId)}, this));
		}
		return true;
	},
	...
	amountCounter : function(id, key)
	{
		if (!BX.util.in_array(id, this.counters[key]))
		{
			this.counters[key].push(id);
			BX.onCustomEvent(this, "onFileIsLoaded", [id, key]);
			if (this.counters.created == (this.counters.images.length + this.counters.files.length) &&
				this.counters.images.length > this.counters.lastShown)
			{
				this.counters.lastShown = this.counters.images.length;
				this.framed = true;
	...
	{
		this.amountCounter(id, (item.dialogName == "BX.UploaderFile" ? "files" : "images"));
	},
	...
	onUploadDone : function(it, data)
	{
		...
		if (item && BX(node))
		{
			...
			input = BX.findChild(node, {tagName : "INPUT", attrs : {id : id}}, true);
			...
			if (file && (item.dialogName == "BX.UploaderImage") !== BX.UploaderUtils.isImage(file["name"], file["type"], file["size"]))
			{
				file["tmp_url"] = uploadId;
				this.replaceFile(item, file, node);
			}
		}
...
//			this.frameFiles();
			...
			if (node && !node.hasAttribute("bx-bound-editor"))
			{
				...
				if (this.agent.dialogName == "BX.UploaderSimple")
				{
					BX.bind(node, "dblclick", BX.delegate(function(e){
						var url = (item.file['real_url'] || item.file['tmp_url']);
						if (url)
							BX.util.popup(url);
						return BX.PreventDefault(e);
					}, this));
	...
	replaceFile : function(item, file, thumb)
	{
		var ii, events = item.__makeThumbEventsObj;
		for (ii in this['fileEvents'])
		{
			if (this['fileEvents'].hasOwnProperty(ii))
			{
		...
		for (ii in item['fields'])
		{
			if (item['fields'].hasOwnProperty(ii))
			{
				if (!!item.fields[ii]["placeHolder"])
				{
					item.fields[ii]["placeHolder"] = null;
					BX.unbindAll(item.fields[ii]["placeHolder"]);
					delete item.fields[ii]["placeHolder"];
				}
			}
		}
		for (ii in events)
		...
		var id = item["id"], node = this.agent.getItem(item.id).node;
		if (!!node)
		{
			if (!!window["jsDD"])
			{
				window.jsDD.unregisterDest(node);
				window.jsDD.unregisterObject(node);
			}
			BX.unbindAll(node);
		}

			...
			<div class="bx-auth-serv-icons" style="display:inline-block">
				...
				foreach($arParams["~AUTH_SERVICES"] as $key=>$service)
				{
					if ($i < $servicesNum)
					{
						...
						?><a title="<?=htmlspecialcharsbx($service["NAME"])?>" href="javascript:void(0)" <?=$onClickEvent?> id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>"><span class="bx-auth-serv-icon <?=htmlspecialcharsbx($service["ICON"])?>"></span></a><?
					}
					else
					{
						if($i == $servicesNum)
						...
						<a title="<?=htmlspecialcharsbx($service["NAME"])?>" href="javascript:void(0)" <?=$onClickEvent?> id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>">
							<span class="bx-auth-serv-icon <?=htmlspecialcharsbx($service["ICON"])?>"></span>
						</a>
						<br/>
					<?
					}

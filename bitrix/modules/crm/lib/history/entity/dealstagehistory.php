...
class DealStageHistoryTable extends Entity\DataManager
{
	...
	public static function markAsJunk($ownerID, $typeID, $borderID = 0)
	{
		if(!is_int($ownerID))
		{
			$ownerID = (int)$ownerID;
		...
		$junkID = HistoryEntryType::JUNK;
		$sql = "UPDATE b_crm_deal_stage_history SET TYPE_ID = {$junkID} WHERE OWNER_ID = {$ownerID} AND TYPE_ID = {$typeID}";
		if($borderID > 0)
		{
			$sql .= " AND ID < {$borderID}";
		}

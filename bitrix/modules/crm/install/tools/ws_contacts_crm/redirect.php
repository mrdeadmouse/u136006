...
$url = substr($_SERVER['REQUEST_URI'], strlen('/bitrix/tools/ws_contacts_crm/'));
...
if (preg_match_all('/[\w]+.aspx\?ID=([\d]+)(.*)/i', $url, $matches))
{
	if ($matches[1][0])
	{
		$url = str_replace('#contact_id#', intval($matches[1][0]), COption::GetOptionString('crm', 'path_to_contact_show', '/crm/contact/show/#contact_id#/'));
	}
	else
	{

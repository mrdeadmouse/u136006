...
if(isset($arUserField['USER_TYPE']))
{
	...
	elseif(is_array($arUserField['USER_TYPE']))
	{
		...
		else if ($arUserField["USER_TYPE"]["USER_TYPE_ID"] == 'integer' || $arUserField["USER_TYPE"]["USER_TYPE_ID"] == 'double'
			|| $arUserField["USER_TYPE"]["USER_TYPE_ID"] == 'datetime')
		{
			$arResult["VALUE"] = array();
			$val1 = '';
			$val2 = '';
			if (isset($_REQUEST[$arUserField["FIELD_NAME"].'_from']))
				$val1 = $_REQUEST[$arUserField["FIELD_NAME"].'_from'];

			if (isset($_REQUEST[$arUserField["FIELD_NAME"].'_to']))
				$val2 = $_REQUEST[$arUserField["FIELD_NAME"].'_to'];

...
if ($iQuoteId > 0)
{
	...
	$arQuote['CONTACT_NAME'] = CUser::FormatName(
		\Bitrix\Crm\Format\PersonNameFormatter::getFormat(),
		array(
			'NAME' => $arQuote['NAME'],
			'LAST_NAME' => $arQuote['LAST_NAME'],
			'SECOND_NAME' => $arQuote['SECOND_NAME']
		),
		true, false
	);
...
<div class="bx-user-info-data-cont-video bx-user-info-fields" id="bx_user_info_data_cont_1">
	...
	if (!empty($arQuote['CONTACT_FULL_NAME']))
	{
		$strCard .= '<span class="field-name">'.htmlspecialcharsbx(GetMessage('CRM_COLUMN_CONTACT_FULL_NAME')).'</span>:
		<a href="'.$arQuote['PATH_TO_CONTACT_SHOW'].'">'.$arQuote['CONTACT_FULL_NAME'].'</a>
		<br />';
	}
	$strCard .= '</div>
</div>';

			...
			if (crumbs instanceof Array)
			{
				...
				for (i = 0; i < crumbs.length; i++)
				{
					...
					if (this.jsEventsMode)
						...
						menuItem["href"] = BX.util.htmlspecialchars(crumbs[i]['LINK']);
				...
				if (showedItems[i]["CHILDREN"] instanceof Array)
				{
					...
					for (j = 0; j < children.length; j++)
					{
						...
						if (this.jsEventsMode)
							...
							menuItem["href"] = BX.util.htmlspecialchars(children[j]["LINK"]);
							...
							BX.create(
								'A',
								{
									attrs: {
										className: "bx-crm-interface-product-section-crumbs-item-link",
										style: "cursor: pointer;",
										href: BX.util.htmlspecialchars(showedItems[i]["LINK"])
									},
									children:
										[
											BX.create(

...
class CCrmViewHelper
{
	...
	public static function RenderWidgetFilterPeriod(array $arParams)
	{
		...
		$periodType = isset($config['periodTypeID'])
			? $config['periodTypeID']
		...
		'function(){ BX.CrmWidgetConfigPeriodEditor.create("', $editorID, '", { config: ', CUtil::PhpToJSObject($config), ', controls: ', CUtil::PhpToJSObject($controls) ,' }); }',
		');',
		'}); </script>';
	}

...
class CAllCrmContact
{
	...
	public static function GetFields($arOptions = null)
	{
		...
		if(!(is_array($arOptions) && isset($arOptions['DISABLE_ADDRESS']) && $arOptions['DISABLE_ADDRESS']))
		{
			$addrJoin = 'LEFT JOIN b_crm_addr ADDR ON L.ID = ADDR.ENTITY_ID AND ADDR.TYPE_ID = '
				.\Bitrix\Crm\EntityAddress::Primary.' AND ADDR.ENTITY_TYPE_ID = '.CCrmOwnerType::Contact;
	...
	public static function CompareFields($arFieldsOrig, $arFieldsModif)
	{
		...
		$arMsg = array_merge(
			$arMsg,
			\Bitrix\Crm\ContactAddress::prepareChangeEvents($arFieldsOrig, $arFieldsModif)
		);
	...
	public static function PrepareFormattedName(array $arFields)
	{
		return CUser::FormatName(
			\Bitrix\Crm\Format\PersonNameFormatter::getFormat(),
			array(

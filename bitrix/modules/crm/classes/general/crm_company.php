...
class CAllCrmCompany
{
	...
	public static function GetFields($arOptions = null)
	{
		...
		if(!(is_array($arOptions) && isset($arOptions['DISABLE_ADDRESS']) && $arOptions['DISABLE_ADDRESS']))
		{
			$addrJoin = 'LEFT JOIN b_crm_addr ADDR ON L.ID = ADDR.ENTITY_ID AND ADDR.TYPE_ID = '
				.\Bitrix\Crm\EntityAddress::Primary.' AND ADDR.ENTITY_TYPE_ID = '.CCrmOwnerType::Company;
			$regAddrJoin = 'LEFT JOIN b_crm_addr R_ADDR ON L.ID = R_ADDR.ENTITY_ID AND R_ADDR.TYPE_ID = '
				.\Bitrix\Crm\EntityAddress::Registered.' AND R_ADDR.ENTITY_TYPE_ID = '.CCrmOwnerType::Company;
	...
	public function Add(array &$arFields, $bUpdateSearch = true, $options = array())
	{
		...
		else
		{
			...
			\Bitrix\Crm\EntityAddress::register(
				CCrmOwnerType::Company,
				$ID,
				\Bitrix\Crm\EntityAddress::Registered,
				array(
		...
		else
		{
			...
			if (isset($arFields['FM']) && is_array($arFields['FM']))
			{
				...
				\Bitrix\Crm\Integrity\DuplicateCommunicationCriterion::register(
					...
					$ID,
					'EMAIL',
					\Bitrix\Crm\Integrity\DuplicateCommunicationCriterion::extractMultifieldsValues($multifields, 'EMAIL')
				);
				\Bitrix\Crm\Integrity\DuplicateCommunicationCriterion::register(
					CCrmOwnerType::Company,
					$ID,
					'PHONE',
					\Bitrix\Crm\Integrity\DuplicateCommunicationCriterion::extractMultifieldsValues($multifields, 'PHONE')
				);
			}
	...
	public static function CompareFields($arFieldsOrig, $arFieldsModif)
	{
		...
		$arMsg = array_merge(
			$arMsg,
			\Bitrix\Crm\CompanyAddress::prepareChangeEvents(
				$arFieldsOrig,
				$arFieldsModif,
				\Bitrix\Crm\EntityAddress::Primary
			)

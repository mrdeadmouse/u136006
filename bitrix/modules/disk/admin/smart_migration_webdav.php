...
class SmartMigrationWebdav
{
	...
	protected function moveUserStorageFromIblock(array $iblock)
	{
		...
		$this->connection->queryExecute("
			...
			SELECT sec.ID, sec.ID, sec.NAME, 2, null, storage.ID, null, sec.DATE_CREATE, sec.DATE_CREATE, sec.CREATED_BY, null, sec.ID, sec.ID, sec.IBLOCK_ID
		...
		$this->connection->queryExecute("
			INSERT INTO b_disk_object (ID, REAL_OBJECT_ID, NAME, TYPE, CODE, STORAGE_ID, PARENT_ID, CREATE_TIME, UPDATE_TIME, CREATED_BY, UPDATED_BY, XML_ID, WEBDAV_SECTION_ID, WEBDAV_IBLOCK_ID)
			SELECT sec.ID, sec.ID, " . $this->getConcatFunction('sec.NAME', "'_'", 'ib.LID') . ", 2, 'FROM_SITE_MOVED', ds.ID, ds.ROOT_OBJECT_ID, sec.DATE_CREATE, sec.DATE_CREATE, sec.CREATED_BY, null, sec.ID, sec.ID, sec.IBLOCK_ID
			FROM b_iblock_section sec
			...
			foreach($propsByIblockId as $iblockId => $props)
			{
				...
				if($this->isMysql)
				{
					$sql = "
						...
						SELECT {$columnForFileId}, {$columnForSize}, {$columnForVersion}, child.NAME, 3, null, parent.STORAGE_ID, child.IBLOCK_SECTION_ID, child.DATE_CREATE, " . $sqlHelper->getIsNullFunction('child.TIMESTAMP_X', 'child.DATE_CREATE') . ", " . $sqlHelper->getIsNullFunction('child.CREATED_BY', 0) . ", child.MODIFIED_BY, child.ID, child.ID, child.IBLOCK_ID
			...
			if($userFieldRow['MULTIPLE'] == 'Y')
			{
				...
				elseif($this->isOracle || $this->isMssql)
				{
					$sql = "
						...
						INNER JOIN b_disk_attached_object attach ON attach.OBJECT_ID = obj.ID AND attach.ENTITY_ID = uf.VALUE_ID AND attach.ENTITY_TYPE = '{$connectorClass}' AND attach.MODULE_ID = '{$moduleId}'
						WHERE
							{$tableNameUf}.ID = uf.ID	AND uf.FIELD_ID = {$userFieldRow['ID']}
						)
			...
			else
			{
				...
				elseif($this->isOracle || $this->isMssql)
				{
					$sql = "
						...
						INNER JOIN b_disk_object obj ON uf.{$userFieldRow['FIELD_NAME']} = obj.WEBDAV_ELEMENT_ID AND obj.WEBDAV_ELEMENT_ID IS NOT NULL
						INNER JOIN b_disk_attached_object attach ON attach.OBJECT_ID = obj.ID AND attach.ENTITY_ID = uf.VALUE_ID AND attach.ENTITY_TYPE = '{$connectorClass}' AND attach.MODULE_ID = '{$moduleId}'
						WHERE {$tableNameSingleUf}.ID = uf.ID)
	...
	protected function mapRightOperation($operationFromIblock)
	{
		$map = array(
			...
			'section_element_bind' => RightsManager::OP_EDIT,
			'section_section_bind' => RightsManager::OP_EDIT,
	...
	protected function fillObjectRights()
	{
		...
		$this->connection->queryExecute("
			INSERT INTO b_disk_right(OBJECT_ID, TASK_ID, ACCESS_CODE, DOMAIN, NEGATIVE)
			SELECT rchild.OBJECT_ID, rtop.TASK_ID, rtop.ACCESS_CODE, '', 1
			...
			WHERE rchild.ACCESS_CODE=rtop.ACCESS_CODE
		"
		);
		...
		$query = $this->connection->query("SELECT DISTINCT TASK_ID FROM b_disk_right");
		while($query && $row = $query->fetch())
		{
			$taskId = $row['TASK_ID'];
			if($newTaskId = $this->convertIblockTaskToDiskTask($taskId))
			{
				...
				else
				{
					if($defaultIblockPermissions[$taskId])
						$defaultUserPermissions[$newTaskId] = $defaultIblockPermissions[$taskId]['GROUP_CODE'];
		...
		Option::set("disk", "default_user_storage_rights", serialize($defaultUserPermissions));
...
//		}
	...
	private function processFinallyActions()
	{
		...
		disk::registerModuleDependences(true);
	}

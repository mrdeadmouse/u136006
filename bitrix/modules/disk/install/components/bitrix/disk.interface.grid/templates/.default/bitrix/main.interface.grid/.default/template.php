...
<?foreach($arResult["HEADERS"] as $header):?>
		<td<?=($header["sort_state"] <> ''? ' class="bx-disk-sorted"':'')?>><div class="empty"></div></td>
<?endforeach?>
	</tr>
...
<?if($arResult["ALLOW_EDIT"] || $arParams["ALLOW_EDIT"]):?>
	<?
	if($aRow["editable"] !== false || $aRow["show_checkbox"] !== false):
...
<?endif?>
	...
	<?endif?>
<?foreach($arResult["HEADERS"] as $id=>$header):?>
		<td<?=($header["sort_state"] <> ''? ' class="bx-disk-sorted"':'')?><?
if($header["align"] <> '')

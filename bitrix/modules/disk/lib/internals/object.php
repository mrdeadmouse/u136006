...
class ObjectTable extends DataManager
{
	...
	public static function checkFields(Result $result, $primary, array $data)
	{
		...
		if(!empty($data['NAME']) && (!IO\Path::validateFilename($data['NAME']) || strpos($data['NAME'], '%') !== false))
		{
			...
			$result->addError(new Entity\FieldError(
				$field,
				Loc::getMessage("DISK_OBJECT_ENTITY_ERROR_FIELD_NAME_HAS_INVALID_CHARS", array("#FIELD#" => $field->getTitle()))
			));
		}
...
//		static::checkFields($result, $primary, $data);
	...
	public static function updateAttributesByFilter(array $attributes, array $filter)
	{
		...
		$event = new Event($entity, self::EVENT_ON_BEFORE_UPDATE_ATTR_BY_FILTER, array(
			'fields' => $attributes,
			'filter' => $filter
		));
		$event->send();
		...
		$event = new Event($entity, self::EVENT_ON_UPDATE_ATTR_BY_FILTER, array(
			'fields' => $attributes,
			'filter' => $filter
		));
		$event->send();
		...
		$event = new Event($entity, self::EVENT_ON_AFTER_UPDATE_ATTR_BY_FILTER, array(
			'fields' => $attributes,
			'filter' => $filter
		));
		$event->send();

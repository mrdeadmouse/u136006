...
class BizProcDocument
{
	...
	public static function updateDocument($documentId, $fields)
	{
		...
		if(!empty($ufFields))
		{
			...
			$USER_FIELD_MANAGER->update('DISK_FILE_' . $storageId, $documentId, $ufFields);
		}
	}
	...
	public static function startAutoBizProc($storageId, $fileId, $autoExecute, $workflowParameters)
	{
		$documentData = array(
			...
			'WEBDAV' => array(
				'DOCUMENT_TYPE' => \Bitrix\Disk\BizProcDocumentCompatible::generateDocumentComplexType($storageId),
	...
	{
		$documentData = array(
			'DISK' => \Bitrix\Disk\BizProcDocument::getDocumentComplexId($fileId),
			'WEBDAV' => \Bitrix\Disk\BizProcDocumentCompatible::getDocumentComplexId($fileId),
		);
		$errors = array();
		foreach($documentData as $nameModule => $data)
		{

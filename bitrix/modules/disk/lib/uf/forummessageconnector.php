...
final class ForumMessageConnector extends StubConnector
{
					...
					switch ($res["EVENT_ID"])
					{
						case "tasks_comment":
							...
							if(Loader::includeModule("tasks"))
							{
								...
								$subData = $connector->getDataToShow($userId);
								if($subData["MEMBERS"])
								{
	...
	public function canAccess($userId, $codes)
	{
		...
		{
			$access = new \CAccess();
			$access->updateCodes(array("USER_ID" => $userId));
	...
	public function canRead($userId)
	{
		...
		if(($res = $this->getDataToCheck($this->entityId)) && !empty($res))
		{
			...
			else if ((!empty($topic["SOCNET_GROUP_ID"]) || !empty($topic["OWNER_ID"])) && Loader::includeModule("socialnetwork"))
			{
			...
			($res2 = $this->getDataToCheck($this->entityId)) && !empty($res2))
		{
			...
			if ($res['DUPLICATE_TO_SOCNET'] == "Y" && $comId > 0 && Loader::includeModule("socialnetwork"))
			{
				if ($res['DUPLICATE_TO_SOCNET'] == "Y")
				{
					...
					$comment_id = \CSocNetLogComments::add($arFieldsForSocnet, false, false, false);
				}
			}

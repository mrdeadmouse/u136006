...
class RightsManager
{
	...
	private function uniqualizeRightsOnObject(array $rights)
	{
		...
		foreach ($rights as $right)
		{
			$uniqueKey = $right['ACCESS_CODE'] . '-' . $right['TASK_ID'] . '-' . (empty($right['DOMAIN'])? '' : $right['DOMAIN']);
			//uniqualize positive rights
			if(!isset($byKey[$uniqueKey]) && empty($right['NEGATIVE']))
			{
		...
		foreach ($rights as $right)
		{
			...
			if(isset($byKey[$uniqueKey]) && !empty($right['NEGATIVE']))
			{
				unset($byKey[$uniqueKey]);
			}
			elseif(!isset($byKey[$uniqueKey]) && !empty($right['NEGATIVE']))
			{
				$byKey[$uniqueKey] = $right;
			}
		}
	...
	private function getUserRightsByObject($objectId, $userId)
	{
		...
		/** @var Query $query */
		$query = new Query(RightTable::getEntity());
		$rights = array_merge($rights, $query
			->setSelect(array(
				'ACCESS_CODE',
				'TASK_ID',
				'NEGATIVE',

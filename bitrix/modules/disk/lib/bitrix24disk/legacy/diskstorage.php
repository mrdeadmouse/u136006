...
class DiskStorage extends AbstractStorage
{
	...
	protected function getSnapshotFromLinks(array $folderLinks, $version)
	{
		...
		foreach($folderLinks as $link)
		{
			...
			$modelLink = FolderLink::buildFromArray(array(
				...
				'UPDATE_TIME' => DateTime::createFromTimestamp($this->convertFromExternalVersion($link['version'])),
			...
			if($version > 0 && $this->compareVersion($link['version'], $version .'000'))
			{
	...
	protected function filterByVersion(array $items, $version = 0)
	{
		...
		{
			if($this->compareVersion($item->getUpdateTime()->getTimestamp() . '000', $version) < 0)
			{
				unset($items[$i]);
			}
		}
	 ...
	 * @throws AccessDeniedException
	 */
	public function addFile($name, $targetDirectoryId, TmpFile $tmpFile)
	{
	...
	public function getVersionDelete($element)
	{
		...
		$v = DeletedLogTable::getList(array('filter' => array(
			'STORAGE_ID' => $this->storage->getId(),

	...
	BX.Mobile.Profile.init({
		...
		menu: [
			...
			},
			{
				...
				action: function () {
					app.openBXTable({
						url: '<?=SITE_DIR?>mobile/webdav/user/<?php echo (int) $arResult["User"]['ID']; ?>/',
						table_settings: {
							type: "files",
							useTagsInSearch: "NO"
						}
...
<div class="emp-profile" id="emp-profile" <?if ($arResult["USER_PERSONAL_PHOTO_SRC"]): ?>style="background-image: url('<?= $arResult["USER_PERSONAL_PHOTO_SRC"]["src"] ?>'); background-size: auto 100%; background-position: center;"<? endif ?>>

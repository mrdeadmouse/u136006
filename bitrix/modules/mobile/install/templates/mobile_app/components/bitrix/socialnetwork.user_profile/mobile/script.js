...
BX.Mobile.Profile = (function() {
	...
	return {
		...
		{
			var button = null;
			if (app.enableInVersion(9) && this.isWebRTCSupported && BX.message("USER_ID") != this.user.id)
			{
				button = {
			...
			for (var i = 0; i < phones.length; i++)
			{
				if (BX.type.isNotEmptyString(phones[i]))
				{
					var item = {
						title: phones[i],
						callback: function() {
							document.location.href = "tel:" + this.title;
						}
					};
		...
		{
			if (BX.message("USER_ID") == this.user.id)
			{
				return null;
			}

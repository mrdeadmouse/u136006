						...
						foreach($arResult["SORT_ITEMS"][$title][$status] as $arItem)
						{
							if ($arItem["PERMISSION"] > "D")
							{
								...
								<li <?if ($title!= "menu-favorites" && in_array($arItem["PARAMS"]["menu_item_id"],$arResult["ALL_FAVOURITE_ITEMS_ID"])):?>style="display:none; " <?endif?>
									...
									<?endif?>
									<a class="menu-item-link" href="<?=($arItem["LINK"]=="/index.php") ? "/" : $arItem["LINK"]?>" onclick="if (B24menuItemsObj.isEditMode()) return false;">
...
<script>
	...
	<? if ($isCompositeMode):?>
	if (!BX.Bitrix24.MenuClass.highlight(document.location.pathname + document.location.search))
	{
		BX.ready(function() {
			BX.Bitrix24.MenuClass.highlight(document.location.pathname + document.location.search);
		});
	}
	<? endif ?>

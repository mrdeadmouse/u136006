...
$arSiteSettings = array('iblock_absence', 'iblock_vacancy', 'iblock_calendar', 'iblock_group_calendar', 'iblock_tasks', /*'search_user_url',*/ 'path_task_group', 'path_task_group_entry', 'path_task_user', 'path_task_user_entry', 'path_user');
...
<?foreach ($arIBTypes as $ibtype_id => $ibtype_name):?><option value="<?echo $ibtype_id?>"<?echo $ibtype_id == $current_ibtype_calendar ? ' selected="selected"' : ''?>><?echo $ibtype_name?></option><?endforeach;?>
	...
	<tr>
		<td valign="top" width="50%"><?echo GetMessage('INTR_OPTION_IBLOCK_TYPE')?>:</td>
		<td valign="top" width="50%"><select name="IBLOCK_TYPE_TASKS" onchange="change_iblock_list(this.value, 3)">
			<option value=""><?echo GetMessage('INTR_OPTION_NOT_SET')?></option>

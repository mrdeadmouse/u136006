		...
		extends CDavGroupdavClient
	{
		...
		private function GetICalContent(array $event, $siteId)
		{
			...
			$arICalEvent = array(
				...
				"SUMMARY" => $event["NAME"],
				"DTSTART" => array(
					"VALUE" => date("Ymd\\THis", CDavICalendarTimeZone::convertDateToTimeZone($event["DT_FROM_TS"], $tzId)),
					"PARAMETERS" => array("TZID" => $tzId)
				),
				"DTEND" => array(
					"VALUE" => date("Ymd\\THis", CDavICalendarTimeZone::convertDateToTimeZone($event["DT_TO_TS"], $tzId)),
					"PARAMETERS" => array("TZID" => $tzId)
				),
			);
			...
			if (isset($event["RRULE"]) && is_array($event["RRULE"]))
			{
				...
				if ($event["DT_SKIP_TIME"] == 'Y')
				{
					...
					$arICalEvent["DTEND"] = array(
						"VALUE" => date("Ymd", CDavICalendarTimeZone::convertDateToTimeZone($event["DT_FROM_TS"] + $event["PROPERTY_EVENT_LENGTH"], $tzId)),
						"PARAMETERS" => array("TZID" => $tzId)
					);
				}
				else
				{
					$arICalEvent["DTEND"] = array(
						"VALUE" => date("Ymd\\THis", CDavICalendarTimeZone::convertDateToTimeZone($event["DT_FROM_TS"] + $event["PROPERTY_EVENT_LENGTH"], $tzId)),
						"PARAMETERS" => array("TZID" => $tzId)
					);
				}
			...
			elseif(isset($event["PROPERTY_PERIOD_TYPE"]) && strlen($event["PROPERTY_PERIOD_TYPE"]) > 0 && ($event["PROPERTY_PERIOD_TYPE"] != "NONE"))
			{
					...
					&& date("H:i:s", MakeTimeStamp($event["ACTIVE_FROM"]) + $event["PROPERTY_EVENT_LENGTH"]) == "00:00:00")
				{
					...
					$arICalEvent["DTEND"] = array(
						"VALUE" => date("Ymd", CDavICalendarTimeZone::convertDateToTimeZone(MakeTimeStamp($event["ACTIVE_FROM"]) + $event["PROPERTY_EVENT_LENGTH"], $tzId)),
						"PARAMETERS" => array("TZID" => $tzId)
					);
				}
				else
				{
					$arICalEvent["DTEND"] = array(
						"VALUE" => date("Ymd\\THis", CDavICalendarTimeZone::convertDateToTimeZone(MakeTimeStamp($event["ACTIVE_FROM"]) + $event["PROPERTY_EVENT_LENGTH"], $tzId)),
						"PARAMETERS" => array("TZID" => $tzId)
					);
				}

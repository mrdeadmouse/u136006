...
BX.Tasks.ui.treeSelector = function(opts, nf){

	this.parentConstruct(BX.Tasks.ui.treeSelector, opts);
	...
	this.handleInitStack(nf, BX.Tasks.ui.treeSelector, opts);
};
BX.extend(BX.Tasks.ui.treeSelector, BX.Tasks.ui.networkIOWidget);
...
BX.merge(BX.Tasks.ui.treeSelector.prototype, {
	...
	init: function(){
		...
		//this.pushFuncStack('buildUpDOM', BX.Tasks.ui.treeSelector);
		this.pushFuncStack('bindEvents', BX.Tasks.ui.treeSelector);
	},
	...
	bindEvents: function(){
		...
		// checkboxes handling
		BX.bindDelegate(sc.scope, 'change', {className: this.getControlClassName('checkbox')}, function(e){
	...
	toggleBundle: function(nodeId, controls){
		...
		var prevState = false;
		var dataTag = 'bx-tasks-ui-'+code+'-state';
	...
	toggleCheckbox: function(control){
		...
		var parent = BX.findParent(control, {className: 'bx-tasks-ui-'+code+'-node'});

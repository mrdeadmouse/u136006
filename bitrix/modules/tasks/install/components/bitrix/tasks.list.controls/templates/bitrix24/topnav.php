	...
	<div class="task-main-wrap" id="task-main-wrap">
		...
		if ($arParams['SHOW_FILTER_BAR'] === 'Y')
		{
			...
			?>
				...
				<span class="task-top-panel-text task-top-panel-switch">
					<a href="<?=CHTTP::urlAddParams($categoryHref, array('SW_FF' => $arResult['FROM_FOR_SWITCH'] == 'FOR' ? 'FROM' : 'FOR'))?>">
						<?=($arResult['FROM_FOR_SWITCH'] == 'FROM' ? GetMessage('TASKS_PANEL_HUMAN_FILTER_STRING_FROM') : GetMessage('TASKS_PANEL_HUMAN_FILTER_STRING_FOR'))?>
					</a>
				</span>
				...
				if ($arResult['SELECTED_SECTION_NAME'] === 'VIEW_SECTION_ROLES') // role filter (4 buttons)
				{
					...
					if ($selectedRoleCodename === 'VIEW_ROLE_RESPONSIBLE')
					{
						echo str_replace(
							...
							array($role, $status, $originatorButtonHTML, $spanText, $spanEnd),
					...
					elseif ($selectedRoleCodename === 'VIEW_ROLE_ORIGINATOR')
					{
						echo str_replace(
							...
							array($role, $status, $responsibleButtonHTML, $spanText, $spanEnd),
					...
					else
					{
						echo str_replace(
							...
							array(
								...
								$arResult['FROM_FOR_SWITCH'] == 'FOR' ? $responsibleButtonHTML : $originatorButtonHTML
							),

...
foreach ($arResult['ITEMS'] as &$arItem)
{
	...
	foreach ($arResult['COLUMNS_IDS'] as $columnId)
	{
		...
		switch($columnId)
		{
			...
			case CTaskColumnList::COLUMN_TITLE:
				...
				<div class="task-title-container">
					...
					<span id="task-title-btn-menu-<?php echo (int) $task['ID']; ?>" class="task-title-btn-wrap">
						...
						<?php
						if (
							$arItem['ALLOWED_ACTIONS']['ACTION_ACCEPT']
			...
			case CTaskColumnList::COLUMN_TIME_ESTIMATE:
				echo sprintf(
					'%02d:%02d:%02d',
					floor($task['TIME_ESTIMATE'] / 3600),		// hours
					floor($task['TIME_ESTIMATE'] / 60) % 60,	// minutes
					$task['TIME_ESTIMATE'] % 60					// seconds
				);
			break;
	...
	<script type="text/javascript"<?php echo $arParams["DEFER"] ? "  defer=\"defer\"" : ""?>>
		...
		<?if($arParams['SHOW_QUICK_INFORMERS'] !== false):?>
			quickInfoData[<?php echo $task["ID"]?>] = <?php tasksRenderJSON($task, $childrenCount, $arParams["PATHS"], false, false, false, $arParams["NAME_TEMPLATE"])?>;
		<?endif?>

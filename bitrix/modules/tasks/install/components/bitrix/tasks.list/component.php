...
elseif ( ! (isset($_GET["F_CANCEL"]) || isset($_GET['F_FILTER_SWITCH_PRESET'])) )
	$arSwitchStateTo = array('sR400');	// default mode
...
$arResult['SUPPORTED_FIELDS_FOR_SORT'] = array(
	...
	'ALLOW_CHANGE_DEADLINE', 'CREATED_DATE', 'CHANGED_DATE',
	'CLOSED_DATE'
);
if (
	isset($_GET["SORTF"])
...
else
{
	if ($sortInOptions === 'none')
	{
		if (
			...
			&& ($arResult['VIEW_STATE']['SECTION_SELECTED']['ID'] == CTaskListState::VIEW_SECTION_ROLES)

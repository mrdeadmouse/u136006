<?
abstract class CAllMeetingItem
{
	...
	public static function Update($ID, $arFields)
	{
		...
		$e = GetModuleEvents('meeting', 'OnBeforeMeetingItemUpdate');
		while ($a = $e->Fetch())
		{
			if (false === ExecuteModuleEventEx($a, array(&$arFields)))
			{
		...
		if (!$bNeedUpdate || $dbRes)
		{
			if (isset($arFields['DESCRIPTION']))
				$DB->QueryBind($query, array('DESCRIPTION' => $arFields['DESCRIPTION']));
			...
			$e = GetModuleEvents('meeting', 'OnAfterMeetingItemUpdate');
			while ($a = $e->Fetch())

					...
					<?php foreach($arResult["ELAPSED_TIME"] as $time):?>
						<tr id="elapsed-time-<?php echo $time["ID"]?>">
							...
							<td class="task-time-spent-column"><?php
								echo floor($time["SECONDS"] / 3600); ?><span><?php echo GetMessage("TASKS_ELAPSED_H"); ?></span><?php
								echo sprintf("%02d", floor($time["SECONDS"] / 60) % 60); ?><span><?php echo GetMessage("TASKS_ELAPSED_M");  ?></span><?php
								echo sprintf("%02d", $time["SECONDS"] % 60); ?><span><?php echo GetMessage("TASKS_ELAPSED_S");
								...
								<div class="wrap-edit-nav">
									...
									<?php if ($time['META:CAN_REMOVE'] || $time['META:CAN_MODIFY']):?>
										<span class="task-edit-nav">
											...
											{
												?>
												<a class="task-table-edit" onclick="EditElapsedTime(<?php echo $time["ID"]?>, <?php echo floor($time["MINUTES"] / 60)?>, <?php echo $time["MINUTES"] % 60?>, '<?php echo CUtil::JSEscape($time["COMMENT_TEXT"])?>', <?php echo $time["SECONDS"] % 60; ?>)"></a>
												<?php
											}
					...
					<?php endforeach?>
						...
						<tr id="task-elapsed-time-form-row" style="display: none;">
							...
							<td class="task-time-spent-column"
								><input type="text" name="HOURS" value="1" /><span><?php
									echo GetMessage("TASKS_ELAPSED_H");
								?></span><input type="text" name="MINUTES" value="00" /><span><?php
...
if ( ! $arResult['IS_IFRAME'] )
{
	$quickBtnOnclick = 'ToggleQuickTask(null';

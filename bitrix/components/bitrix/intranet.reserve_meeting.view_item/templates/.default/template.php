	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%" class="intask-main data-table">
		<tbody>
			<tr>
				...
				<td width="60%"><?=FormatDate($arParams['DATE_TIME_FORMAT'], strtotime($arResult["ITEM"]["DATE_ACTIVE_FROM"]))?></td>
			</tr>
			<tr>
				<td align="right"><?= GetMessage("INTASK_C29T_DATE_TO") ?>:</td>
				<td><?=FormatDate($arParams['DATE_TIME_FORMAT'], strtotime($arResult["ITEM"]["DATE_ACTIVE_TO"]))?></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td valign="top"><?
					if ($arResult["ITEM"]["PROPERTY_PERIOD_TYPE_VALUE"] != "NONE")
					{
						...
						echo FormatDate($arParams['DATE_TIME_FORMAT'], strtotime($arResult["ITEM"]["DATE_ACTIVE_TO_FINISH"]));
					}
					else
					{

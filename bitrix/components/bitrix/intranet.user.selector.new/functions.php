...
class CIntranetUserSelectorHelper
{
	public static function drawEmployeeStructure($arStructure, $arSections, $key, $name, $bSelectSection = false)
	{
		foreach ($arStructure[$key] as $id)
		{
			...
			if ($bSelectSection)
			{
				echo '<span class="company-department-inner" id="'.$name.'_employee_section_'.$id.'" data-section-id="'.$id.'">';
				echo '<div class="company-department-arrow" onclick="O_'.$name.'.load(\''.$id.'\', false, false, true)"></div>';
				echo '<div class="company-department-text" onclick="O_'.$name.'.selectSection(\''.$name.'_employee_section_'.$id.'\')">'.htmlspecialcharsbx($arRes['NAME']).'</div>';
				echo '</span>';
			}
			else
			{

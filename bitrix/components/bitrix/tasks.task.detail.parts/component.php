...
try
{
	...
	elseif (
		($arParams['MODE'] === 'JUST AFTER TASK CREATED')
	...
	if (
		isset($arParams['TASK_ID'])
		&& (
			in_array('buttons', $arResult['BLOCKS'])
			|| in_array('right_sidebar', $arResult['BLOCKS'])
		)
	)
	{
		...
		if (
			($arResult['TIMER'] !== false)
			&& ($arResult['TIMER']['TIMER_STARTED_AT'] > 0)
			&& ($arResult['TIMER']['TASK_ID'] == $arParams['TASK_ID'])
		)
		{
			$arResult['TIMER_IS_RUNNING_FOR_CURRENT_USER'] = 'Y';
		}
		else
		{
			$arResult['TIMER_IS_RUNNING_FOR_CURRENT_USER'] = 'N';
		}

		if (
			($arResult['TIMER_IS_RUNNING_FOR_CURRENT_USER'] === 'Y')
			|| $arResult['ALLOWED_ACTIONS']['ACTION_START_TIME_TRACKING']
			|| ($arResult['TASK']['ALLOW_TIME_TRACKING'] === 'Y')
		)
		{

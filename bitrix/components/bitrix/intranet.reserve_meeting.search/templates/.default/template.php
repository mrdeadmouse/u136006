	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%" class="intask-main data-table">
		...
		<tbody>
			...
			<tr class="intask-row">
				...
				<td class="intask-cell">
					<input type="text" name="flt_uf_place" value="<?= HtmlSpecialCharsbx($_REQUEST["flt_uf_place"]) ?>" size="5">
				</td>
			</tr>
			<tr class="intask-row">
				...
				<td class="intask-cell">
					...
					$GLOBALS["APPLICATION"]->IncludeComponent(
						...
						array(
							...
							'INPUT_VALUE' => (StrLen($_REQUEST["flt_date_from"]) > 0 ? $_REQUEST["flt_date_from"] : Date($GLOBALS["DB"]->DateFormatToPHP(FORMAT_DATE))),
							'SHOW_TIME' => 'N',
					...
					$GLOBALS["APPLICATION"]->IncludeComponent(
						...
						array(
							...
							'INPUT_VALUE' => (StrLen($_REQUEST["flt_date_to"]) > 0 ? $_REQUEST["flt_date_to"] : Date($GLOBALS["DB"]->DateFormatToPHP(FORMAT_DATE))),
							'SHOW_TIME' => 'N',
							'INPUT_ADDITIONAL_ATTR' => $strAdd,
						),
						null,
			...
			<tr class="intask-row">
				...
				<td class="intask-cell">
					...
					<select name="flt_time_from">
						<?
						$s = (StrLen($_REQUEST["flt_time_from"]) > 0 ? $_REQUEST["flt_time_from"] : "08:00");
						for ($i = 0; $i < 48; $i++)
						{
							$t = IntVal($i / 2);
							if ($t < 10)
					...
					<select name="flt_time_to">
						<?
						$s = (StrLen($_REQUEST["flt_time_to"]) > 0 ? $_REQUEST["flt_time_to"] : "19:00");
						for ($i = 0; $i < 48; $i++)
						{
							$t = IntVal($i / 2);
							if ($t < 10)
